		...
		try {
			...
			ComputationGraphConfiguration discriminatorConfig = new NeuralNetConfiguration.Builder().seed(seed)
					...
					.addLayer("D_2",
							...
							"D_1")
					.addLayer("D_final",
							new DenseLayer.Builder().nIn(128).nOut(2).activation(Activation.LEAKYRELU)
									.weightInit(WeightInit.XAVIER).updater(updaterD).build(),
							"D_2")
					/* -------------------------D------------------------- */
			...
			for (int i = 0; i < epochs; i++) {
				...
				INDArray ones = Nd4j.ones(dataType, new long[] { num, 1 });
				INDArray zeros = Nd4j.zeros(dataType, new long[] { num, 1 });

				INDArray inputZ = Nd4j.randn(dataType, new long[] { num, vectorSize });
				INDArray labelDx = Nd4j.hstack(zeros, ones);
				INDArray labelDgz = Nd4j.hstack(ones, zeros);
				INDArray labelDgzT = Nd4j.hstack(zeros, ones);
