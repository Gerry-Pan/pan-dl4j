...
public class WassersteinConvGanTrainer {
	...
	protected static double lr = 0.0002;
		...
		try {
			...
			if (file.exists()) {
				...
				ComputationGraphConfiguration graphConfiguration = new NeuralNetConfiguration.Builder().seed(seed)
						...
						.addLayer("Gz_conv_3",
								new ConvolutionLayer.Builder(3, 3).updater(updaterG).activation(Activation.TANH)
										.nOut(channels).build(),
...
//						.addLayer("D_dropout_2", new DropoutLayer(0.25), "D_activation_2")
	...
	protected static void draw(ComputationGraph discriminator, MnistDataSetIterator testDataSetIterator,
